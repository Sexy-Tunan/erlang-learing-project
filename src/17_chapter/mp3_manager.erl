%% SHOUTcast服务发送的数据流有以下特点
%% F H F H F H F H
%% F是一个MP3音频数据块，它的长度必须刚好是24 576字节（icy-metaint参数的值）。H是
%% 一个数据头，由单字节的K后接16*K字节的数据组成。因此，可以用二进制型表示的最小数据头
%% 是<<0>>。接下来的数据头可以这样表示：
%% <<1,B1,B2,...,B16>>
...
%% 
%% 
%% (1) 制作一个播放列表。我们在16.2.5节的“读取MP3元数据”部分创建了一个包含歌名清单
%% 的文件，服务器将使用这个文件。音频文件将从清单里随机选择。 
%% (2) 制作一个并行服务器，这样就能并行传输多个流。我们将使用17.1.3节中“并行服务器”部分描述的方法。 
%% (3) 我们只想发送各个音频文件的音频数据到客户端，而不包括内嵌的ID3标签。音频编码器
%% 理应能跳过损坏的数据，所以原则上可以把ID3标签和数据一起发送。然而在实践中，似乎需要
%% 移除ID3标签才能让程序更好地运行。
%% 
%% 
-module(mp3_manager).
